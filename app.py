...
st.caption("上傳 Excel/CSV -> 欄位對應/清理 -> 關係拆解與配比 -> 儀表板 -> 匯出")
...
with tab_dash:
    # ---- Sidebar filters (affecting all widgets in this tab) ----
    ...
    c1.metric("水電公司數量", f"{filtered['水電公司'].nunique()}")
    c2.metric("經銷商數量", f"{filtered['經銷商'].nunique()}")
    c3.metric("經銷商承接總量(萬元)", fmt_num(filtered['承接量_萬'].sum(), 0))
    ...
    with st.expander("TOP 經銷商承接量", expanded=True):
        topN = st.slider("顯示前 N 名", 5, 30, 10, 1, key="top_dealer")
